...
def adminDeleteCommodityDescription(requestParameters):
    ...
    cur.execute("""UPDATE  commodity_description_table 
    SET status = %(status)s , comm_desc = %(comm_desc)s ,commodity_id = %(commodity_id)s 
    WHERE comm_desc_id=%(comm_desc_id)s;""",
                {"status": status, "comm_desc": comm_desc, "comm_desc_id": comm_desc_id, "commodity_id": commodity_id})
    cur.close()
    conn.commit()
