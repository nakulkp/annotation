...
def fetchMapping(requestParameters):
    ...
    for row in rows:
        ...
        categories = fetchCategory({'is_null' : category_id})
        commodities = fetchCommodity({'is_null' : commodity_id})
        demand_value = fetchDemand({'is_null' : demand_value_id})
        factor_value = fetchFactorValue({'is_null' : factor_value_id})
        moving_factors = fetchMovingFactor({'is_null' : moving_factor_id})
        price_value = fetchPrice({'is_null' : price_value_id})
        countries = fetchRegion({'is_null' : country_id})
        sc_disruption_value = fetchSCDisruption({'is_null' : sc_disruption_value_id})
        sub_categories = fetchSubCategory({'is_null' : subcategory_id})
        supply_value = fetchSupply({'is_null' : supply_value_id})

        returnList.append(categories)
        returnList.append(commodities)
        returnList.append(demand_value)
        returnList.append(factor_value)
        returnList.append(moving_factors)
        returnList.append(price_value)
        returnList.append(countries)
        returnList.append(sc_disruption_value)
        returnList.append(sub_categories)
        returnList.append(supply_value)
