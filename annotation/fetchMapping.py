...
def fetchMapping(requestParameters):
    ...
    for row in rows:
        ...
        categories = fetchCategory(category_id)
        commodities = fetchCommodity(commodity_id)
        demand_value = fetchDemand(demand_value_id)
        factor_value = fetchFactorValue(factor_value_id)
        moving_factors = fetchMovingFactor(moving_factor_id)
        price_value = fetchPrice(price_value_id)
        countries = fetchRegion(country_id)
        sc_disruption_value = fetchSCDisruption(sc_disruption_value_id)
        sub_categories = fetchSubCategory(subcategory_id)
        supply_value = fetchSupply(supply_value_id)
